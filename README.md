Digital Blossom
Smart Connected Flower Pot — Technical Specification
...
An interactive, long-distance relationship desktop art piece and IoT device, “Digital Blossom” features a circular color IPS display mounted on a hollow metallic stem resembling a flower blossom. This is paired with a smart electronics hub, physical controls, an integrated battery charger, a secondary dashboard status display, and a two-way digital voice mailbox—all nestled inside a compact desktop planter pot.
This document details the complete firmware architecture, mechanical specifications, and assembly instructions required to build, flash, and configure the Digital Blossom system.
1.1 Key Features
•	“Touch of Love” Presence / Virtual Water Drop: A capacitive touch sensor wired to a brass leaf or pot rim detects physical interaction. Touching the leaf registers a timestamp on a shared real-time database. If your partner touches theirs within 10 seconds, both devices trigger a synchronized, vibrant “Together Now” golden blooming petal animation accompanied by a harmonic chime.
...
To successfully handle dual-buffered graphic layouts, secure webhooks, and digital audio streams, staying within the ESP32-S3 family with external PSRAM is required. Below is the complete bill of materials optimized to stay within a €30 budget.
2.1 Core Processor & Power
...
 
3. ESP32-S3 System Pinout Mapping
The abundant GPIO capacity of the ESP32-S3 DevKitC-1 allows all components to connect simultaneously. Refer to the hardware mapping table below for wiring configurations:
               ...
               +----------------------------+    |    |
                                           ...
                                           [Slide Switch]
                                               ...
                                               (5V) (GND)
                                                 ...
                                                 v    v
                                            ESP32-S3 Board
Figure 1: Power Distribution and Charging Schematic
 
5. Firmware Setup & Development
The software architecture is split across both CPU cores on the ESP32-S3 to prevent networking routines from blocking user interface animations.
5.1 Core Allocation
•	Core 0 (Network & Background Daemons): Handles WiFiManager setups, secure Telegram Bot long-polling/Firebase endpoints, file synchronization in LittleFS, and RTC time updates.
•	Core 1 (UI Engine & Interaction): Drives the round GC9A01 display using LVGL (v8.3+) with hardware DMA acceleration, renders the OLED dashboard using the U8g2/Adafruit libraries, handles rotary interrupts, and decodes incoming I2S audio streams.
5.2 Required Software Libraries
Install the following libraries using the Arduino IDE Library Manager (Ctrl+Shift+I):
1.	TFT_eSPI: Optimized SPI library for the GC9A01 LCD.
2.	Adafruit_SSD1306 & Adafruit_GFX: For driving the dashboard OLED.
3.	UniversalTelegramBot & ArduinoJson: For parsing Telegram messages, photos, and voice notes.
4.	RTClib: For synchronization and reading from the DS3231 RTC.
5.	lvgl (v8.3.x): Embedded Light and Versatile Graphics Library for the blossom interface.
5.3 Compiling and Verifying
To check code syntax before your components arrive, follow these steps:
6.	Open your Arduino IDE.
7.	Go to File > Preferences and add the following URL to the Additional Boards Manager URLs field:
https://raw.githubusercontent.com/espressif/arduino-esp32/gh-pages/package_esp32_index.json
8.	Go to Tools > Board > Boards Manager, search for esp32, and install the library package.
9.	Set your board target to ESP32-S3 DevKitC-1 under Tools > Board.
10.	In the settings, ensure PSRAM is configured as OPI PSRAM to match the WROOM-1 N16R8 module specifications.
11.	Verify your code by clicking the Verify (Checkmark) icon on the top-left of the IDE interface.
6. Getting Started: Tonight's Action Plan
Do not wait for your physical components to arrive to get started on the Digital Blossom! Follow these immediate steps to lay the groundwork:
12.	Secure Your Cloud Space: Contact the @BotFather on Telegram, create your custom bot, and save your secure API key. Use the @IDBot to retrieve your Chat ID.
13.	Setup Your Environment: Install the Arduino IDE, download the Espressif ESP32 package, and pre-install the libraries listed in Section 5.2. Write the core connection and JSON skeleton files.
14.	Layout Enclosure Dimensions: Draft your planter pot design on paper or in 3D CAD software (such as Tinkercad or Fusion360) ensuring the base compartment is at least 85mm wide to accommodate the 18650 holder and DevKitC-1.
15.	Draft Your Assets: Write your 365 daily quotes, crop relationship pictures into 240x240 circular crops, and test layout grids.
